<?php
	class Tracks_List_Table extends WP_List_Table {
			...
			function extra_tablenav ($where)
			{
				...
				echo '<input id="addtrack" class="button action" type="submit" value="Upload track" name="">';
				echo '</div>';
			}
			...
			function prepare_items ()
			{
				...
				$orderby = 'tstart';
				if (!empty ($_REQUEST ['orderby']) && in_array ($_REQUEST ['orderby'], array ('id','name','tstart','tend','source'))) {
				...
				//$sql = 'SELECT x.total, id, name, created, source, comment FROM '. $this -> options ['tbl_tracks'] .
				//	', (select count(*) as total FROM '. $this -> options ['tbl_tracks'] . ") as x WHERE user_id='". get_current_user_id().
				//	"' ORDER BY $orderby $order LIMIT $offset,$limit";

				$sql = "SELECT t.id, t.name, t.source, t.comment, min(l.occurred) as tstart, max(l.occurred) as tend, count(l.occurred) as numpoints FROM ".
